...
frequency_dict = {}
for lat, city in latitudes_and_cities:
    original_lat=lat
    lat_key = round(10000 * lat)
    if lat_key not in frequency_dict:
        frequency_dict[lat_key] = {'frequency': 0, 'cities': []}
    frequency_dict[lat_key]['frequency'] += 1
    frequency_dict[lat_key]['cities'].append(city)

# Extract keys, frequencies, and city names for each latitude
keys = list(frequency_dict.keys())
freq = [item['frequency'] for item in frequency_dict.values()]
city_names = [item['cities'] for item in frequency_dict.values()]
